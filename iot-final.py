...
def get_door_status(filename="./status.txt"):
    """
    :param filename: the status file, we don't need to pass it
    :return: the status of the door, 1 - open, 0 - close
    this function is used to simulate getting door status.
    """
...
def set_door_status(status, filename="./status.txt"):
    """
    :param status:
    :param filename:
    :return: none
    this function is to simulate controlling the door status.
    """
    print("writing the " + status + " to " + filename)
...
def customShadowCallback_Delta(payload, responseStatus, token):
    ...
    set_door_status(payloadDict['state']['status'])
...
def iot_shadow():
    ...
    while True:
        ...
        if i % 100 == 0 or newstatus != status:
